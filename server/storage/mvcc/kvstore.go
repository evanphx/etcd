...
type store struct {
	...
	// (len(key)+len(value)) of every live key in the store. Unlike the
	// backend's physical Size(), it is unaffected by revision history,
	// tombstones, or an LSM's compaction backlog, so it reflects the true
	// keyspace size. It is maintained incrementally on put/delete and rebuilt on
	// restore, and read atomically (writes happen under the write-txn lock).
	// The pebble-backed quota uses it instead of physical size (see
	// storage.NewBackendQuota) to avoid spuriously tripping NOSPACE on Pebble's
